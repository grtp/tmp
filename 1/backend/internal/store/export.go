...
func ValidateFilters(def *meta.TableDef, filters map[string]string) (string, []any, error) {
	return buildWhere(def, "", filters)
}
...
// 返り値は出力した行数(監査記録用)。
func (s *TableStore) ExportCSV(ctx context.Context, def *meta.TableDef, filters map[string]string, w io.Writer) (int, error) {
	db, err := s.dbs.DB(ctx, def.ConnectionID)
	if err != nil {
		return 0, err
	}

	where, args, err := buildWhere(def, "", filters)
	if err != nil {
		return 0, err
	}
