...
const TEXT_KEYS: HistoryFilterKey[] = [
  'username',
  'operation',
  'target',
  'clientIp',
...
@Component({
  ...
  imports: [MatButtonModule, MatIcon, TranslocoPipe, TmResizeColumnsDirective],
...
export class HistoryPage {
  ...
  /** 機能フィルタの選択肢(組込機能のコード) */
  readonly actionCodes = input<string[]>([
    'auth',
    'table-maint',
    'settings',
    'history',
  ...
  readonly csvExportClicked = output<HistoryRow[]>();
  /** [JSONをダウンロード]押下(overflow 行は取得込みでコンテナがDLする) */
  readonly downloadRequested = output<number>();

  protected readonly PAGE_SIZES = [10, 20, 50, 100, 500, 1000];

  /** 複数選択モード(CSV の[選択範囲出力]用。トグル中だけチェック列を出す) */
  protected readonly multiSelectMode = signal(false);
  /** チェック中の行 id。行が入れ替わったらリセット */
  ...
  protected toggleMultiSelect(): void {
    ...
    if (!this.multiSelectMode()) this.selected.set(new Set());
  }
  ...
  protected toggleAll(checked: boolean): void {
    this.selected.set(
      checked ? new Set(this.entries().map((e) => e.id)) : new Set(),
    );
  }

  /**
   * ドラッグ範囲選択(data-table-page と同じ機構)。
   * チェックボックス列に限らず行の上からドラッグ開始できる
   * (そのまま離せば通常の行クリック，動かせば範囲選択)。
   */
  private dragAnchor: number | null = null;
  private dragActive = false;
  /** ドラッグ直後の click(詳細開閉/checkbox トグル)を 1 回だけ抑止する */
  ...
  /** 行クリック: 複数選択モード中はチェックのトグル，通常時は詳細開閉。 */
  protected onRowClick(e: HistoryRow): void {
    if (this.suppressClick) {
      this.suppressClick = false;
      return;
    }
    if (this.multiSelectMode()) {
      this.toggleRow(e.id, !this.selected().has(e.id));
      return;
    }
    this.toggle(e.id);
  }
  ...
  protected readonly allChecked = computed(
    () =>
      ...
      this.entries().every((e) => this.selected().has(e.id)),
  );

  protected selectedRows(): HistoryRow[] {
    const sel = this.selected();
    return this.entries().filter((e) => sel.has(e.id));
  }
