// Package appdb は本ツール自身のテーブル(ftool_app_*)が置かれるスキーマを一元管理する。
//
// ソース上のアプリDB向けSQLは従来どおり dbo.ftool_app_xxx と書き，実行直前に
...
// conn/meta の ftool_app_ 参照)のSQLに現れる `dbo.` が ftool_app_ テーブルの
...
//     側の責務であり，本パッケージは「どのスキーマの ftool_app_* を読むか」の
//     実行時解決のみを担う。
package appdb
