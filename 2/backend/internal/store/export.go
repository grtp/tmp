...
// ValidateFilters checks column filters only(列の存在・値の型)。
// ストリーム系ハンドラがヘッダー送信前に 400 を返すために使う。
func ValidateFilters(def *meta.TableDef, filters map[string]string, preds []predicate.Predicate) (string, []any, error) {
	return buildWhere(def, "", filters, preds)
}

// ExportCSV は列フィルタ一致の全行を CSV でストリーム出力する。
// 列は def.Columns 全列(rowversion は含めない)，ヘッダー行 = 列名。
// セル表現: NULL=空文字 / bool=true・false / date=YYYY-MM-DD / datetime=RFC3339。
// 返り値は出力した行数(監査記録用)。
func (s *TableStore) ExportCSV(ctx context.Context, def *meta.TableDef, filters map[string]string, preds []predicate.Predicate, w io.Writer) (int, error) {
	db, err := s.dbs.DB(ctx, def.ConnectionID)
	if err != nil {
		return 0, err
	}

	where, args, err := buildWhere(def, "", filters, preds)
	if err != nil {
		return 0, err
	}
