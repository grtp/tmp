...
func (s *TableStore) ListRows(ctx context.Context, def *meta.TableDef, p ListParams) (*RowPage, error) {
	...
	where, args, err := buildWhere(def, p.Q, p.Filters, p.Preds)
	if err != nil {
		return nil, err
	}
...
// buildWhere は全体検索 q(文字列列の OR LIKE)・列フィルタ・述語(いずれも
// AND 結合)を組み立てる。
func buildWhere(def *meta.TableDef, q string, filters map[string]string, preds []predicate.Predicate) (string, []any, error) {
	var andConds []string
	var args []any
	...
	// チップフィルタの述語(演算子・列内OR・否定)。
	if len(preds) > 0 {
		conds, pargs, err := predicate.Build(predSpecs(def), preds, len(args))
		if err != nil {
		...
		andConds = append(andConds, conds...)
