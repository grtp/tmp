...
try {
  ...
  // チップフィルタ: 旧フィルタ行は撤去済み。バーは既定で非表示で,
  // じょうごを押すと行が現れて直接列選択ポップオーバーが開く
  check('old filter row is gone', (await page.$('.filter-row')) === null);
  check('filter bar hidden by default', (await page.$('tm-filter-bar')) === null);
  await page.click('.filter-toggle');
  await sleep(400);
  check(
    'funnel opens picker directly',
    (await page.$('.filter-bar-row')) !== null &&
      (await page.$('tm-filter-bar .col-search')) !== null,
  );
  // ポップオーバーは fixed でじょうごの真下(パネルの overflow に見切れない)
  const popPos = await page.evaluate(() => {
    const funnel = document.querySelector('.filter-toggle').getBoundingClientRect();
    const pop = document.querySelector('tm-filter-bar .popover');
    const r = pop.getBoundingClientRect();
    return {
      position: getComputedStyle(pop).position,
      underFunnel: Math.abs(r.top - (funnel.bottom + 4)) < 2,
      leftOk: r.left <= funnel.left + 1,
      insideViewport: r.bottom <= window.innerHeight && r.left >= 0,
    };
  });
  check(
    'picker is fixed under the funnel inside the viewport',
    popPos.position === 'fixed' && popPos.underFunnel && popPos.leftOk && popPos.insideViewport,
  );
  await page.keyboard.press('Escape');
  await sleep(400);
  check('bar disappears when closed without applying', (await page.$('.filter-bar-row')) === null);
  ...
  // 7) 設定 > 機能マスタ: settings 行は disabled のチェックで固定表示(操作不可) + API 409
  ...
  await page.evaluate(() => {
    ...
    tr?.querySelector('.icon-btn[aria-label="編集"]')?.dispatchEvent(new MouseEvent('click', { bubbles: true }));
  });
  await sleep(500);
  ...
  await sleep(500);
  await clickByText(page, 'button.primary', '追加');
  await sleep(500);
  await page.evaluate(() => {
    const dlg = document.querySelector('tm-managed-table-dialog');
