...
# 2) cp deploy/.env.example deploy/.env して本番値を書く
#    (キーの説明は .env.example。開発の deploy/.env.dev と同じ形)
# 3) フロントをビルドして nginx にマウントさせる
make generate && make fe_install && make fe_build
podman compose -f deploy/compose.yaml up -d --build
# 4) 受け口の決定
#    - このスタックの nginx を直接使う      : .env で FRONTEND_PORT=80
#    - ホストに既存の nginx がある(2段構成): 既定の 8081 のままにし，
#      deploy/nginx/host-frontend.conf.example をホスト nginx へ配置
#      (運用・保守手順書 §1.3)
```
