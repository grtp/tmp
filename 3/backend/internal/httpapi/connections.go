...
func (s *Server) ListConnections(c *gin.Context) {
	...
	if err != nil {
		internalError(c, "list connections", err, "接続一覧の取得に失敗しました")
		return
	}
...
func (s *Server) CreateConnection(c *gin.Context) {
	...
	s.auditOK(c, authz.ActionSettings, "connection.create", "conn:"+created.Name,
		gin.H{"id": created.ID, "host": created.Host, "database": created.DatabaseName,
...
func (s *Server) UpdateConnection(c *gin.Context, id int) {
	...
	if req.Password != nil && *req.Password == "" {
		...
		badRequest(c, "password を空にはできません(変更しない場合は送らないでください)")
		return
	}
	...
	s.auditOK(c, authz.ActionSettings, "connection.update", "conn:"+updated.Name,
		gin.H{"id": id, "passwordChanged": req.Password != nil})
...
func (s *Server) DeleteConnection(c *gin.Context, id int) {
	...
	s.auditOK(c, authz.ActionSettings, "connection.delete", "conn:"+target.Name,
		gin.H{"id": id, "host": target.Host, "database": target.DatabaseName})
...
func (s *Server) writeConnError(c *gin.Context, err error, fallback string) {
	...
	default:
		internalError(c, "conn", err, fallback)
	}
}
