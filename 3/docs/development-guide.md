...
- **httpapi 内の約束**: ハンドラは「認可 → 入力検証 → 委譲 → 監査 → 応答」に
  ...
  (`users.go` / `actions.go` / `history.go` / `dashboard.go` …)
...
4. FK 設計の注意(SQL Server):
   - **複数カスケードパス**があると FK 作成自体が失敗する
     (例: users→dash_templates と users→user_dash_items が両方 CASCADE で
     衝突するケース)。その場合は NO ACTION にする
...
変わると権限・ダッシュボード項目・個人テンプレートの紐付けが切れる(移行時は
