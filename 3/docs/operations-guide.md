...
1. `deploy/.env` の `FRONTEND_PORT` を 8081(既定)のままにする
   — コンテナ nginx はホストの 8081 で待つ
2. ひな形をホスト nginx へ置き，ホスト名と転送先を直す

```bash
cp deploy/nginx/host-frontend.conf.example /etc/nginx/conf.d/ftool.conf
vi /etc/nginx/conf.d/ftool.conf        # server_name / proxy_pass のポート
...
- コンテナ nginx の公開先は既定で `127.0.0.1`(`FRONTEND_BIND`)。
  ...
  `FRONTEND_BIND=0.0.0.0` にしたうえで，`deploy/nginx/default.conf` の
  `X-Forwarded-For` も `$remote_addr` へ変える(そこが境界になるため)


ホスト nginx を使わず，コンテナ nginx を唯一の受け口にする場合は
`.env` で `FRONTEND_PORT=80` にする(rootless podman で 1024 未満が使えない
環境ではポートを上げるか `net.ipv4.ip_unprivileged_port_start` を調整する)。
