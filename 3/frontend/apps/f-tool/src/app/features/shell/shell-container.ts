...
export class ShellContainer {
  ...
  /**
   * ヘッダー時計の秒表示(個人設定)。個人設定ダイアログと同じストアを
   * 見るので，変えた瞬間にヘッダーへ反映される。
   */
  protected readonly clockSeconds = this.userSettings.headerClockSeconds;
  ...
  protected readonly menuItems = computed<MenuItem[]>(() => {
    ...
    const items: MenuItem[] = [
      { id: 'home', label: t('dashboard.menuHome'), icon: 'home' },
    ];
    if (this.auth.allows('table-maint', 'user')) {
      items.push({
        id: 'table-maint',
        label: t('dashboard.menuTableMaint'),
        icon: 'table_view',
      });
    }
  ...
  private openPersonalSettings(): void {
    ...
    ref.componentInstance.headerClockSecondsChanged.subscribe((on: boolean) => {
      ref.componentRef?.setInput('errorMessage', null);
      void this.userSettings
        .setHeaderClockSeconds(on)
        .catch((err) => {
