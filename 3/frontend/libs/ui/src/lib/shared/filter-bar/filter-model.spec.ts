...
describe('toPredsParam', () => {
  it('空配列は undefined', () => {
    ...
    const param = toPredsParam([{ column: 'a', op: 'eq', values: ['1'] }]);
    if (!param) throw new Error('expected a defined param');
    expect(JSON.parse(param)).toEqual([{ column: 'a', op: 'eq', values: ['1'] }]);
  });
});
