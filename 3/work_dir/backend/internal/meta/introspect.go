...
// Introspection is the catalog-derived shape of one physical table.
...
// Introspect reads columns/PK/rowversion for schema.table from the catalog.
...
// CandidateTable is one row of the registration dialog listing.
...
// ListCandidates lists BASE TABLEs that can be registered: excludes the
// tool's own app_% tables and everything already in app_managed_tables.
func ListCandidates(ctx context.Context, db *sql.DB, schemaFilter string) ([]CandidateTable, error) {
	const q = `
...
WHERE t.TABLE_TYPE = 'BASE TABLE'
  ...
  AND NOT EXISTS (
      SELECT 1 FROM dbo.app_managed_tables m
      WHERE m.schema_name = t.TABLE_SCHEMA AND m.table_name = t.TABLE_NAME)
