// Package conn manages the registered target SQL Server connections:
// encrypted credential storage (app_connections), DSN construction, and
// a lazily-populated pool of *sql.DB keyed by connection id.
//
...
// LoadKey decodes the base64 CONN_ENC_KEY and requires exactly 32 bytes
...
// Encrypt returns nonce || AES-256-GCM(plaintext).
...
func Decrypt(key, blob []byte) ([]byte, error) {
	if len(blob) < gcmNonceSize+16 { // nonce + GCM tag
