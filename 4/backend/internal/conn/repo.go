...
// CreateParams / UpdateParams だけがパスワードを運ぶ。
...
const connCols = `id, name, host, port, database_name, username, COALESCE(options, N''), enabled, created_by`

func scanConn(row interface{ Scan(...any) error }) (*Connection, error) {
	var c Connection
	err := row.Scan(&c.ID, &c.Name, &c.Host, &c.Port, &c.DatabaseName,
		&c.Username, &c.Options, &c.Enabled, &c.CreatedBy)
...
func (r *Repo) List(ctx context.Context) ([]Connection, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT `+connCols+` FROM dbo.app_connections ORDER BY name`)
	if err != nil {
...
func (r *Repo) Get(ctx context.Context, id int) (*Connection, error) {
	return scanConn(r.db.QueryRowContext(ctx,
		`SELECT `+connCols+` FROM dbo.app_connections WHERE id = @p1`, id))
}
...
func (r *Repo) Create(ctx context.Context, p CreateParams) (*Connection, error) {
	...
	err = r.db.QueryRowContext(ctx, `
		INSERT INTO dbo.app_connections
			(name, host, port, database_name, username, password_enc, options, enabled, created_by)
		OUTPUT INSERTED.id
		VALUES (@p1, @p2, @p3, @p4, @p5, @p6, @p7, @p8, @p9)`,
		p.Name, p.Host, p.Port, p.DatabaseName, p.Username, enc,
		nullIfEmpty(p.Options), p.Enabled, p.CreatedBy).Scan(&id)
	if err != nil {
...
func (r *Repo) Update(ctx context.Context, id int, p UpdateParams) (*Connection, error) {
	var sets []string
	var args []any
	...
	q := fmt.Sprintf(`UPDATE dbo.app_connections SET %s, updated_at = SYSUTCDATETIME() WHERE id = @p%d`,
...
// Delete refuses when app_managed_tables still references the connection.
// FK 制約は最後の安全網で、通常はここで ErrInUse を返す。
func (r *Repo) Delete(ctx context.Context, id int) error {
	var refs int
	if err := r.db.QueryRowContext(ctx,
		`SELECT COUNT(*) FROM dbo.app_managed_tables WHERE connection_id = @p1`, id).Scan(&refs); err != nil {
		return fmt.Errorf("conn: ref count: %w", err)
	}
	if refs > 0 {
		return fmt.Errorf("%w (%d件)", ErrInUse, refs)
	}
	res, err := r.db.ExecContext(ctx, `DELETE FROM dbo.app_connections WHERE id = @p1`, id)
	if err != nil {
...
// DSN loads and decrypts the connection and builds a sqlserver:// DSN.
...
func (r *Repo) DSN(ctx context.Context, id int) (string, error) {
	...
	err := r.db.QueryRowContext(ctx, `
		...
		FROM dbo.app_connections WHERE id = @p1`, id).
...
// BuildDSN constructs a go-mssqldb URL DSN with escaped credentials.
