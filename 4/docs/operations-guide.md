...
環境A で `FRONTEND_PORT=80` が使えない場合(rootless podman では 1024 未満を
直接使えないことがある)は, ポートを上げるか
...
```bash
IMAGE_TAG=2026-07-30 podman compose -f deploy/compose.yaml build
```
