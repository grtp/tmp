...
CREATE TABLE ftoolapp.ftool_app_dash_templates (
    id          INT IDENTITY(1,1) NOT NULL CONSTRAINT pk_ftool_app_dash_templates PRIMARY KEY,
    owner_guid  UNIQUEIDENTIFIER NULL CONSTRAINT fk_ftool_app_dash_templates_owner REFERENCES ftoolapp.ftool_app_users(object_guid),
    name        NVARCHAR(128) NOT NULL,
    description NVARCHAR(400) NULL,
...
CREATE TABLE ftoolapp.ftool_app_dash_template_items (
    id          INT IDENTITY(1,1) NOT NULL CONSTRAINT pk_ftool_app_dash_template_items PRIMARY KEY,
    template_id INT NOT NULL CONSTRAINT fk_ftool_dash_items_template REFERENCES ftoolapp.ftool_app_dash_templates(id) ON DELETE CASCADE,
    position    INT NOT NULL,
    kind        NVARCHAR(8) NOT NULL CONSTRAINT ck_ftool_dash_items_kind CHECK (kind IN (N'action', N'link', N'table')),
    action_id   INT NULL CONSTRAINT fk_ftool_dash_items_action REFERENCES ftoolapp.ftool_app_actions(id) ON DELETE CASCADE,
    managed_table_id INT NULL CONSTRAINT fk_ftool_dash_items_mtable REFERENCES ftoolapp.ftool_app_managed_tables(id) ON DELETE CASCADE,
...
CREATE INDEX ix_ftool_dash_items_template ON ftoolapp.ftool_app_dash_template_items(template_id, position);
GO

CREATE TABLE ftoolapp.ftool_app_user_dash_items (
    id          INT IDENTITY(1,1) NOT NULL CONSTRAINT pk_ftool_app_user_dash_items PRIMARY KEY,
    object_guid UNIQUEIDENTIFIER NOT NULL CONSTRAINT fk_ftool_user_dash_items_user REFERENCES ftoolapp.ftool_app_users(object_guid) ON DELETE CASCADE,
    position    INT NOT NULL,
    kind        NVARCHAR(8) NOT NULL CONSTRAINT ck_ftool_user_dash_items_kind CHECK (kind IN (N'action', N'link', N'table')),
    action_id   INT NULL CONSTRAINT fk_ftool_user_dash_items_action REFERENCES ftoolapp.ftool_app_actions(id) ON DELETE CASCADE,
    managed_table_id INT NULL CONSTRAINT fk_ftool_user_dash_items_mtable REFERENCES ftoolapp.ftool_app_managed_tables(id) ON DELETE CASCADE,
    name        NVARCHAR(128) NULL,
    url         NVARCHAR(512) NULL,
    icon        NVARCHAR(64)  NULL,
    created_by  NVARCHAR(256) NOT NULL,
...
CREATE INDEX ix_ftool_user_dash_items_user ON ftoolapp.ftool_app_user_dash_items(object_guid, position);
GO
