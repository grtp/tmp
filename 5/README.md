...
※「F-tool」は表示名(仮)。コード名(Go モジュール `tablemaint`、`app_` テーブル、
リポジトリ構成)は変更しない方針。表示名は i18n リソース(`common.systemName`)で差し替え可能。
favicon は `frontend/apps/table-maint/public/favicon.svg`(六角ナット + F、SVG)。
...
| 層 | 技術 |
|---|---|
| frontend | Angular 21 (Nx monorepo)。UI は `libs/ui` に分離し Storybook で管理。各コンポーネントは template/styles を外部ファイル(`*.html` / `*.css`)に分離 |
...
| `APP_DB_SCHEMA` | dbo | app_ テーブルを置くスキーマ。DB側の都合で専用スキーマを使う場合に変更(initdb の `:setvar APP_SCHEMA` と揃える) |
