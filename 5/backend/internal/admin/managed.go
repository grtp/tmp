...
const managedCols = `
	...
	m.readonly_columns, m.hidden_columns, m.sort_order,
...
func scanManaged(row interface{ Scan(...any) error }) (*ManagedTable, error) {
	...
	err := row.Scan(&m.ID, &connID, &m.ConnectionName, &m.SchemaName, &m.TableName,
		...
		&m.ReadonlyColumns, &m.HiddenColumns, &m.SortOrder, &m.Enabled, &m.CreatedBy)
	...
	err := r.queryRow(ctx, `
		INSERT INTO dbo.app_managed_tables
			(connection_id, schema_name, table_name, display_name, description, pk_columns,
			 readonly_columns, hidden_columns, sort_order, created_by)
		OUTPUT INSERTED.id
		VALUES (@p1, @p2, @p3, @p4, @p5, @p6, @p7, @p8, @p9, @p10)`,
		connArg, m.SchemaName, m.TableName, m.DisplayName, nullIfEmpty(m.Description),
		m.PKColumns, m.ReadonlyColumns, m.HiddenColumns, m.SortOrder, m.CreatedBy).Scan(&id)
	if err != nil {
...
func (r *Repo) UpdateManagedTable(ctx context.Context, id int, displayName, description *string, sortOrder *int, enabled *bool, readonlyColumns, hiddenColumns *string) (*ManagedTable, error) {
	var sets []string
	var args []any
	...
	if readonlyColumns != nil {
		add("readonly_columns", *readonlyColumns)
	}
	if hiddenColumns != nil {
		add("hidden_columns", *hiddenColumns)
	}
	if len(sets) == 0 {
		return r.GetManagedTable(ctx, id)
	}
...
func (r *Repo) DeleteManagedTable(ctx context.Context, id int) error {
	res, err := r.exec(ctx, `DELETE FROM dbo.app_managed_tables WHERE id = @p1`, id)
	if err != nil {
