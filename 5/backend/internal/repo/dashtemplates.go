...
// DashTemplate is one dashboard template (配布 or 個人所有).
...
func (r *Repo) ListDashTemplates(ctx context.Context, includeDisabled bool, viewerGUID string) ([]DashTemplate, error) {
	q := `
		...
		FROM dbo.ftool_app_dash_templates`
	var args []any
	if includeDisabled {
	...
	if err != nil {
		return nil, fmt.Errorf("repo: list dash templates: %w", err)
	}
	defer rows.Close()
...
func (r *Repo) GetDashTemplate(ctx context.Context, id int, viewerGUID string) (*DashTemplate, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("repo: get dash template: %w", err)
	}
	...
	rows, err := r.query(ctx, `
		...
		FROM dbo.ftool_app_dash_template_items i
	...
	if err != nil {
		return nil, fmt.Errorf("repo: get dash template items: %w", err)
	}
	defer rows.Close()
...
func (r *Repo) CreateDashTemplate(ctx context.Context, t DashTemplate) (*DashTemplate, error) {
	...
	var id int
	err := r.queryRow(ctx, `
		INSERT INTO dbo.ftool_app_dash_templates (owner_guid, name, description, enabled, created_by, updated_by)
		OUTPUT INSERTED.id
		VALUES (CAST(@p1 AS UNIQUEIDENTIFIER), @p2, @p3, @p4, @p5, @p5)`,
	...
	if err != nil {
		...
		return nil, fmt.Errorf("repo: create dash template: %w", err)
	}
	return r.GetDashTemplate(ctx, id, t.OwnerGUID)
}

func (r *Repo) UpdateDashTemplate(ctx context.Context, id int, name, description *string, enabled *bool, viewerGUID, updatedBy string) (*DashTemplate, error) {
	var b setBuilder
	if name != nil {
		b.add("name", *name)
	}
	...
	q := fmt.Sprintf(`UPDATE dbo.ftool_app_dash_templates SET %s, updated_at = SYSUTCDATETIME() WHERE id = @p%d`,
		b.clause(), b.arg(id))
	...
	if err != nil {
		...
		return nil, fmt.Errorf("repo: update dash template: %w", err)
	}
...
func (r *Repo) DeleteDashTemplate(ctx context.Context, id int) error {
	// items は CASCADE。実体化コピー方式のため ftool_app_user_dash_items には
	// 影響しない(適用済みユーザーのカードは独立して残る)。
	res, err := r.exec(ctx, `DELETE FROM dbo.ftool_app_dash_templates WHERE id = @p1`, id)
	if err != nil {
		return fmt.Errorf("repo: delete dash template: %w", err)
	}
	if n, _ := res.RowsAffected(); n == 0 {
		return ErrNotFound
	}
...
// ReplaceDashTemplateItems replaces all items(配列順 = position)。
// actingUser は created_by/updated_by に使う(全置換なので毎回新規行になり,
// created_by も updated_by も同じ actor でよい)。
func (r *Repo) ReplaceDashTemplateItems(ctx context.Context, templateID int, items []DashTemplateItem, viewerGUID, actingUser string) (*DashTemplate, error) {
	...
	if _, err := tx.ExecContext(ctx,
		appdb.Q(`DELETE FROM dbo.ftool_app_dash_template_items WHERE template_id = @p1`), templateID); err != nil {
	...
	for pos, it := range items {
		...
		if _, err := tx.ExecContext(ctx, appdb.Q(`
			INSERT INTO dbo.ftool_app_dash_template_items
				(template_id, position, kind, action_id, managed_table_id, name, url, icon, created_by, updated_by)
			VALUES (@p1, @p2, @p3, @p4, @p5, @p6, @p7, @p8, @p9, @p9)`),
