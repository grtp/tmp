...
本書は F-tool(コード名 `tablemaint`)へ機能追加・変更を行う開発者向けの手順書。
...
│   └─ apps/table-maint/ ルーティング・API 層・コンテナ(配線)
├─ deploy/     compose(dev/本番)、initdb/01_init.sql(DDL の正)
...
wsl -d alma9 -- podman start tm-mssql tm-redis tm-ldap
#   初回や作り直しは: make dev-up  (podman compose -f deploy/compose.dev.yaml up -d)
#   tm-mssql が (healthy) になるまで待つ:
wsl -d alma9 -- podman ps

# 2) バックエンド(:8080)
make run-dev-ldap        # OpenLDAP 実物で認証(通常はこちら)
# make run-dev           # モック認証 admin/admin(LDAP 不要の簡易確認用)

# 3) フロントエンド開発サーバー(:4200。/api は :8080 へプロキシ)
make fe-serve
```

開発ユーザー(OpenLDAP): `tanaka` / `suzuki` / `sato`(パスワード共通 `Passw0rd!`)。
tanaka に admin×3(table-maint / settings / history)を付与して使うのが基本。

### 2.3 開発環境の既知の注意点

| 事象 | 対処 |
|---|---|
| WSL 再起動でコンテナが止まる | `podman start tm-mssql tm-redis tm-ldap` で再開 |
| コンテナ再作成で **LDAP の entryUUID が変わり権限が消える** | 権限を再付与(設定>ユーザー権限、または MERGE の SQL)。`make dev-down` はボリュームごと消すので特に注意 |
| dev サーバーが**リビルドを止めて古いバンドル**を配る(i18n が生キー表示等) | :4200 のプロセスを kill して `make fe-serve` し直す。判定は必ずフレッシュなブラウザで行う |
...
3. **i18n は ja/en 同時に追加**(`libs/ui/src/lib/i18n/{ja,en}.json`)。
...
7. **DDL は `deploy/initdb/01_init.sql` が正**。新規 CREATE と、既存DB向けの
   **冪等 ALTER 追補**(`IF COL_LENGTH(...) IS NULL` 等)を必ずセットで書く。
   - app_ テーブルのスキーマは `$(APP_SCHEMA)` 変数で書く(冒頭の
     `:setvar APP_SCHEMA dbo`。`dbo.app_x` と直書きしない)
...
8. **アプリDBへの SQL は `dbo.app_xxx` と書き、必ず appdb.Q を通す**
   (admin/conn は Repo の `query/queryRow/exec` ヘルパ経由、tx 内は
...
1. 変更箇所の特定
   - 見た目・DOM: `frontend/libs/ui/src/lib/<component>/*.html|css|ts`
   - データ取得・保存・遷移: `frontend/apps/table-maint/src/app/features/<機能>/`
...
5. 検証:
   ```bash
   cd frontend
   npx nx build table-maint --skip-nx-cache
...
1. `apps/table-maint/src/app/features/<name>/` にコンテナ(ts/html)を作成。
...
2. `make generate`(または backend/ で
   `go run github.com/oapi-codegen/oapi-codegen/v2/cmd/oapi-codegen@v2.4.1 -config ../api/oapi-codegen.yaml ../api/openapi.yaml`)。
   生成型のフィールドはポインタになりがちなので nil チェックを忘れない。
3. backend 実装:
   - データアクセス: `internal/admin/`(app_ テーブル)または `internal/store/`
     (動的テーブル)
   - ハンドラ: `internal/httpapi/` に生成インターフェースのメソッドを実装。
     冒頭で認可、成功・失敗の監査、エラーは `abortErr` /
     `writeAdminError` / `writeStoreError` で HTTP へ写像
   - `go build ./... && go test ./...`
4. frontend 実装:
   - 型: `apps/table-maint/src/app/core/models.ts`(OpenAPI を写す)
...
1. `deploy/initdb/01_init.sql` を変更:
...
2. 開発DBへ適用:
   ...
   wsl -d alma9 -- podman cp <file.sql> tm-mssql:/tmp/x.sql
   wsl -d alma9 -- podman exec tm-mssql /opt/mssql-tools18/bin/sqlcmd \
     -S localhost -U sa -P 'DevPassw0rd!' -C -d tablemaint -b -i /tmp/x.sql
   ```
...
#### b. 既定接続(アプリ自身のDB)の変更
- 開発: `make run-dev*` の環境変数(`DB_HOST/DB_PORT/DB_NAME/DB_USER/DB_PASSWORD`
  または `MSSQL_DSN`)
- 本番: `deploy/.env` の `MSSQL_DSN` を変更して backend を再起動
- **app_ テーブルを dbo 以外のスキーマに置く場合**: `APP_DB_SCHEMA` を設定し、
  initdb は `:setvar APP_SCHEMA` を同じ値にして実行(運用・保守手順書 §1.2b)
...
1. `libs/ui/src/lib/i18n/<lang>.json` を追加(ja.json を複製して翻訳)。
...
2. 履歴 CSV の人間可読要約が必要なら
   `backend/internal/admin/history_summary.go` の `summarizeDetail` に分岐を追加。
3. 履歴画面の「機能」フィルタ select は
   `libs/ui/src/lib/history-page/history-page.ts` の `actionCodes` 既定値に追加。
...
```bash
cd frontend
npx nx build table-maint --skip-nx-cache      # 型チェック込み
npx nx build-storybook ui --skip-nx-cache     # ストーリーの破壊検知
```

### 5.3 ブラウザ E2E(リグレッション一式)

`e2e/e2e.mjs`(120+ チェック)。前提: コンテナ+backend(:8080)+
fe-serve(:4200)が起動済みであること。
...
注意:
- E2E は開始時に **tanaka のダッシュボード設定・個人カードをリセット**する。
  手動検証と並走させない(データが競合して誤爆する)
- シードデータ(bigdata / products / customers / codes / wide、
  demoDB の audit_demo=列モード検証用)に依存するチェックがある
...
curl -s -c $cj -X POST http://127.0.0.1:8080/api/v1/auth/login \
  -H 'Content-Type: application/json' -d '{"username":"tanaka","password":"Passw0rd!"}'
...
2. DDL 変更がある場合: `01_init.sql` の**追補ブロックだけ**を本番 SQL Server へ
   適用(冪等なので再実行可。事前に app_ テーブルをバックアップ)
3. ビルドと反映:
   ```bash
