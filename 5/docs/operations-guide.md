...
| 変数 | 既定 | 説明 |
|---|---|---|
| `MSSQL_DSN` | 開発値 | アプリDB接続 `sqlserver://user:pass@host:1433?database=tablemaint` |
| `APP_DB_SCHEMA` | dbo | app_ テーブルを置くスキーマ(§1.2b)。initdb の `:setvar APP_SCHEMA` と揃える |
...
- **認証は SQL Server 認証のサービスアカウント**を使う(Linux コンテナからの
  Windows 統合認証は運用しない)。専用アカウント例: `svc_tablemaint`
...
- **専用スキーマを割り当てられた場合**(DB 側の都合で app_ テーブルを
  `dbo` 以外に置く場合):
  1. `deploy/initdb/01_init.sql` 冒頭の `:setvar APP_SCHEMA dbo` を
     指定スキーマ名に書き換えて実行(スキーマは自動作成される)
  2. backend の環境変数 `APP_DB_SCHEMA` に同じスキーマ名を設定
  3. 起動時の存在確認(VerifyAppTables)もこのスキーマを見る。不一致だと
     「required table <schema>.app_users not found」で fail-fast する
- 必要な DB 権限(最小権限):
  - 初期セットアップ/DDL 追補の適用時のみ: 対象DBの `db_ddladmin` 相当
    (専用スキーマ運用なら、そのスキーマ限定の CONTROL でも可)
  - 通常運用: app_ テーブルへの `db_datareader` + `db_datawriter` で足りる
...
1. SQL Server にアプリDB(例 `tablemaint`)を作成し、
   `deploy/initdb/01_init.sql` の **app_ テーブル部分**を実行
   (スクリプトは冪等。再実行しても安全)。
...
6. 管理者となるユーザーで一度ログイン(app_users に自動登録される)
...
### 5.2 アプリDB(app_ テーブル)の移設

1. 計画停止(backend 停止)
2. 旧サーバーの app_ テーブル群をバックアップ → 新サーバーへ復元
...
ディレクトリ移行で GUID が変わる場合、権限・個人カード・個人テンプレートの
紐付けが切れる。移行時は `app_users.object_guid` の新旧対応表を作り
UPDATE する計画を立てること(未対応のまま移行するとユーザーは
「初回ログイン扱い」になり閲覧権限のみになる)。
...
- 保持期間: 現状は無期限。規程が決まり次第、アーカイブテーブル+定期移動
  ...
  当面のサイズ監視: `SELECT COUNT(*) FROM dbo.app_operation_history;`
...
| 対象 | 方法 | 備考 |
|---|---|---|
| アプリDB(app_ テーブル群) | SQL Server の標準バックアップに含める | 設定・権限・**監査履歴**を含む。最重要 |
...
2. アプリDBをバックアップ
3. DDL 追補(あれば)を適用 — `01_init.sql` の追補ブロックは冪等
4. `make generate && make fe-install && make fe-build`
5. `podman compose -f deploy/compose.yaml up -d --build`
