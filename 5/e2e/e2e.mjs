...
try {
  ...
  await sleep(400);
  await page.click('tm-page-header .title');
  await page.waitForFunction(() => location.pathname === '/dashboard', { timeout: 10000 });
  ...
  // 4) 並べ替え: FAB→[並び替え]でモード開始 → D&D → 決定で保存 / キャンセルで破棄
  ...
  await clickByText(page, 'tm-add-feature-dialog button.entry', 'テンプレート');
  await sleep(300);
  await clickByText(page, 'tm-add-feature-dialog button.entry', '並び替え');
  await sleep(300);
  ...
  await page.goto(BASE + '/dashboard', { waitUntil: 'networkidle2' });
  await sleep(600);
  check('brand logo (hex nut) in header', (await page.$('.header-title .brand-logo')) !== null);
