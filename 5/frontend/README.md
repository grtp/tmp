...
```
apps/table-maint/        アプリ本体 (Angular 21.2 / esbuild)
libs/ui/                 UI コンポーネントライブラリ (A案デザイン)
  src/lib/login-page/        ログイン画面 + stories
  src/lib/dashboard-page/    ダッシュボード + stories
  src/lib/data-table-page/   メタデータ駆動テーブル + stories
  src/styles/tokens.css      デザイントークン (--tm-*, メイン #3e69ad)
  .storybook/                この lib 専用の Storybook 設定
```

アプリ側からは `import { LoginPage } from '@table-maint/ui';`
...
- tokens.css は Storybook では project.json の storybook /
  ...
  アプリ本体で使う場合は apps/table-maint/project.json の build
