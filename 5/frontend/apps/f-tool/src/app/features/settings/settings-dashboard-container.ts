// features/settings — 設定>ホーム設定(/settings/dashboard)。
// 配布テンプレートの CRUD(機能マスタは /settings/functions に分離)。
import {
...
export class SettingsDashboardContainer {
  ...
  protected readonly visibleTabs = signal<SettingsTab[]>(['templates']);
  protected readonly tab = signal<SettingsTab>('templates');
