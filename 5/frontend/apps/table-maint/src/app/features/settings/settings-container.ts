...
const SECTIONS: Record<string, { tabs: SettingsTab[]; titleKey: string }> = {
  ...
  dashboard: { tabs: ['actions', 'templates'], titleKey: 'pages.settingsDashboard' },
...
@Component({
  ...
  imports: [SettingsPage, ManagedTableDialog, ConnectionDialog, TemplateEditorDialog, ConfirmDialog, PageHeader, TranslocoPipe],
...
export class SettingsContainer {
  ...
  protected readonly editorActions = computed<EditorAction[]>(() =>
    this.actions()
      ...
      .map((a) => ({ id: a.id, code: a.code, name: a.name, icon: a.icon })),
  );
