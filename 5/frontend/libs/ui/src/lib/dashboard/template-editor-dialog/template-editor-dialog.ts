...
/** カード追加のビュー。'form' が基底で,スタックに積んだ分だけ ← で戻れる。 */
type ViewId = 'form' | 'root' | 'action' | 'table' | 'link';

/**
 * ダッシュボードテンプレートの作成/編集ダイアログ(admin)。
 * 名前/説明/有効 + カード構成を ↑↓ で並べて保存する。
 *
 * カード追加は add-feature-dialog と同じ階層ナビゲーション:
 *
 *   form ─ [+ 項目を追加] ─ root ─ 機能を追加 ── 機能1..n(選択で追加し form へ)
 *                                ├ テーブルカードを追加 ─ テーブル1..n(同上)
 *                                └ リンクを追加(名前+URL フォーム → 追加で form へ)
 *
 * 種類が増えても root に入口を1行足すだけで拡張できる。
 */
@Component({
...
export class TemplateEditorDialog {
  ...
  /** 追加ナビゲーションの遷移スタック(基底 form は含まない)。 */
  protected readonly stack = signal<Exclude<ViewId, 'form'>[]>([]);

  protected readonly view = computed<ViewId>(() => {
    const s = this.stack();
    return s.length > 0 ? s[s.length - 1] : 'form';
  });
  ...
  protected push(view: Exclude<ViewId, 'form'>): void {
  ...
  protected titleKey(): string {
    switch (this.view()) {
      case 'root':
        return 'templateEditor.addItem';
      case 'action':
        return 'templateEditor.addAction';
      case 'table':
        return 'templateEditor.addTable';
      case 'link':
        return 'templateEditor.addLink';
      default:
        return this.mode === 'create'
          ? 'templateEditor.createTitle'
          : 'templateEditor.editTitle';
    }
  }

  protected addAction(a: EditorAction): void {
  ...
  protected addTable(t: EditorTable): void {
    this.items.update((xs) => [
      ...xs,
      {
        kind: 'table',
  ...
  /** disableClose で Esc は無効化されているため自前処理。追加中は1つ戻る。 */
