 ...
 *
 *   <table [tmResizeColumns]="'forge.colw:settings:users'">
 *
...
export class TmResizeColumnsDirective implements AfterViewInit, OnDestroy {
  ...
  private rebuild(): void {
    ...
    let fillerCol = colgroup.querySelector(`col[${FILLER_MARK}]`) as HTMLTableColElement | null;
    ...
    ths.forEach((th, i) => {
      ...
      handle.addEventListener('pointerenter', () => {
        handle.style.background = 'linear-gradient(to left, rgba(62,105,173,0.55) 3px, transparent 3px)';
      });
      handle.addEventListener('pointerleave', () => {
        handle.style.background = '';
      });
      handle.addEventListener('pointerdown', (e) => this.startResize(e, th, col, i));
      handle.addEventListener('dblclick', () => this.resetWidth(th, col, i));
  ...
  private autoFitIfNeeded(table: HTMLTableElement, ths: HTMLTableCellElement[]): void {
    ...
    const sig = ths.map((th, i) => this.keyOf(th, i)).join(',') + '|' + (hasData ? 'd' : 'e');
    if (sig === this.fitSignature) return;
    this.fitSignature = sig;

    const colgroup = table.querySelector(':scope > colgroup');
    if (!colgroup) return;
    const colEls = (Array.from(colgroup.children) as HTMLTableColElement[]).filter(
      (c) => !c.hasAttribute(FILLER_MARK),
    );
    ...
    const measured = ths.map((th) =>
      Math.min(AUTOFIT_MAX, Math.max(AUTOFIT_MIN, Math.ceil(th.getBoundingClientRect().width) + 2)),
    );
  ...
  private resetWidth(th: HTMLTableCellElement, col: HTMLTableColElement, index: number): void {
