...
/** 保存イベント。[複製]で編集→新規に切り替わるため mode を同梱する。 */
...
export class RowEditDialog {
  ...
  /** [複製]で 'edit' → 'create' に切り替わるため signal で持つ。 */
  ...
  /**
   * [複製]: 主キーと rowversion(予約キー $rowVersion)を除いた値をコピーして
   * 新規作成のドラフトに切り替える(DB には保存されない。[保存]で insert)。
   */
  protected duplicate(): void {
    this.draft.update((d) => {
      ...
      for (const c of this.columns) {
        if (c.primaryKey) copy[c.name] = null;
      }
      return copy;
    });
