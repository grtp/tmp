...
// GetUserDash は本人の設定を返す(行が無ければゼロ値)。
func (r *Repo) GetUserDash(ctx context.Context, guid adguid.GUID) (*UserDash, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("admin: get user dash: %w", err)
	}
	if tid.Valid {
		v := int(tid.Int64)
		d.TemplateID = &v
...
func (r *Repo) SetUserDash(ctx context.Context, guid adguid.GUID, d UserDash) error {
	if d.TemplateID != nil {
		...
		if err != nil {
			return fmt.Errorf("admin: check template: %w", err)
		}
	}
...
WHEN NOT MATCHED THEN
	...
	if _, err := r.exec(ctx, q, string(guid), tid, order, hidden); err != nil {
		return fmt.Errorf("admin: set user dash: %w", err)
	}
	return nil
}
