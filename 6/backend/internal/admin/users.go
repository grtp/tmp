...
// ListUsers returns all users (q = username/display_name の部分一致) with
// their per-action levels.
func (r *Repo) ListUsers(ctx context.Context, q string) ([]UserWithAuth, error) {
	...
	var args []any
	if q != "" {
		args = append(args, "%"+escapeLikeAdmin(q)+"%")
	...
	if err != nil {
		return nil, fmt.Errorf("admin: list user auth: %w", err)
	}
	defer arows.Close()
	for arows.Next() {
...
func (r *Repo) GetUser(ctx context.Context, guid adguid.GUID) (*UserWithAuth, error) {
	...
	err := r.queryRow(ctx, `
		SELECT LOWER(CONVERT(NVARCHAR(36), object_guid)), username, display_name,
		       COALESCE(email, N''), last_login_at
		FROM dbo.app_users WHERE object_guid = CAST(@p1 AS UNIQUEIDENTIFIER)`, string(guid)).
	...
	if err != nil {
		return nil, fmt.Errorf("admin: get user auth: %w", err)
	}
	defer rows.Close()
	u.Auth = []UserAuthEntry{}
	for rows.Next() {
...
// ReplaceUserAuth replaces every grant of the user in one transaction
...
func (r *Repo) ReplaceUserAuth(ctx context.Context, guid adguid.GUID, assignments []AuthAssignment, updatedBy string) error {
	...
	if _, err := tx.ExecContext(ctx,
		appdb.Q(`DELETE FROM dbo.app_user_auth WHERE object_guid = CAST(@p1 AS UNIQUEIDENTIFIER)`),
		string(guid)); err != nil {
		return fmt.Errorf("admin: clear auth: %w", err)
	}
	for _, a := range assignments {
		...
		if _, err := tx.ExecContext(ctx, appdb.Q(`
			...
			VALUES (CAST(@p1 AS UNIQUEIDENTIFIER), @p2, @p3, @p4)`),
			string(guid), a.ActionID, string(a.AuthLevel), updatedBy); err != nil {
			// FK 違反 = 存在しない action_id
			return fmt.Errorf("admin: insert auth (action %d): %w", a.ActionID, err)
		}
	}
	return tx.Commit()
}
...
func escapeLikeAdmin(s string) string { return likeEscaper.Replace(s) }
