...
// BlobStore は Put/Get だけを要求する(internal/blobstore.Store が実装)。
// テストしやすいよう最小のインターフェースにしている。
...
// NewRecorder の blobStore は nil を渡してよい(退避機能を使わない構成)。
...
func (r *Recorder) overflow(ctx context.Context, full []byte) []byte {
	...
	if err := r.blobStore.Put(ctx, key, full, "application/json"); err != nil {
		log.Printf("audit: overflow put failed: %v", err)
