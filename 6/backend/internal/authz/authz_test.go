...
func TestGrantsAllows(t *testing.T) {
	g := testGrants(map[string]Level{
		ActionTables: LevelUser,
		ActionHistory:    LevelMaintainer,
	})

	if !g.Allows(ActionTables, LevelUser) {
		t.Error("tables:user は user を満たすはず")
	}
	if g.Allows(ActionTables, LevelMaintainer) {
		t.Error("tables:user は maintainer を満たさないはず")
	}
