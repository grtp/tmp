...
type Config struct {
	...
	// TrustedProxies は X-Forwarded-For を信じてよい前段プロキシ(CIDR か IP)。
	// 監査履歴の client_ip はこのヘッダから解決するため,ここに載っていない
	// 相手から来たヘッダは無視して接続元をそのまま使う。
	// 空 = どのプロキシも信じない(前段が無い開発環境の既定)。
	// 本番はコンテナ nginx のサブネット(compose で固定した 10.89.0.0/24)を指定する。
	TrustedProxies []string

	// --- ログ ---
	// LogFormat: "text"(既定。podman logs を人が読む前提) | "json"
	// LogLevel : debug / info / warn / error
	LogFormat string
	LogLevel  string // dev(ng serve)専用。本番はnginx同一オリジンのため未使用。

	// --- SQL Server ---
	MSSQLDSN string // "sqlserver://user:pass@host:1433?database=ftool"
	// AppDBSchema: 本ツール自身のテーブル(ftool_app_*)を置くスキーマ。
...
// envList はカンマ区切りの環境変数を分解する。空要素は捨てる
// (末尾カンマや空白だけの設定で意図せぬ値が混ざらないように)。
// 未設定・空文字なら nil を返す。
