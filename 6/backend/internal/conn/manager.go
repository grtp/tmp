...
// Manager caches one *sql.DB per registered connection, opened lazily.
//
//   - connID == nil -> 既定接続(アプリ自身の DB。main が所有し Close しない)
//   - connID != nil -> app_connections から DSN を構築して遅延 open + キャッシュ
//
...
// DB returns the pool for a connection id (nil = default).
