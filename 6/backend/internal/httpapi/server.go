...
type Server struct {
	...
	conns    *conn.Repo
	pools    *conn.Manager
	// アプリ自身の DB(app_* テーブル用)
	db *sql.DB
	// 監査履歴 detail の 1MB 超過分の取得用(nil = 未設定。§S11)
	blobs BlobGetter
}

// BlobGetter は監査履歴 overflow の読み出しにのみ使う最小インターフェース
...
// login, slides the Redis TTL, resolves per-action grants, and stores
// both in the gin context.
	...
	return func(c *gin.Context) {
		...
		if err != nil {
			if !errors.Is(err, session.ErrNotFound) {
				// Redis 障害も fail-closed(401)。理由はログにだけ残す。
