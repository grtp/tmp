...
func TestNormalizeColumnModes(t *testing.T) {
	...
	t.Run("hidden+fixed combination allowed", func(t *testing.T) {
		fixed := []FixedColumn{{Name: "note", Kind: "literal", Value: "v", ApplyOn: "both"}}
		_, hd, fxJSON, err := NormalizeColumnModes(testIntrospection(), nil, []string{"NOTE"}, fixed)
		if err != nil {
			t.Fatalf("hidden+fixed should be allowed: %v", err)
		}
		if hd != "Note" || !strings.Contains(fxJSON, `"name":"Note"`) {
			t.Errorf("got hd=%q fxJSON=%s", hd, fxJSON)
		}
	})

	t.Run("readonly+fixed still rejected", func(t *testing.T) {
		fixed := []FixedColumn{{Name: "note", Kind: "literal", Value: "v", ApplyOn: "both"}}
		_, _, _, err := NormalizeColumnModes(testIntrospection(), []string{"note"}, nil, fixed)
		if err == nil || !strings.Contains(err.Error(), "複数のモード") {
			t.Errorf("readonly+fixed dup should fail: %v", err)
		}
	})
