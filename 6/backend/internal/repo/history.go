...
// LastActivityByTarget は target ごとの最終操作日時(成功分のみ)を
// 一括で返す(管理対象テーブル一覧のカードに「最終操作」を出すため。
// テーブルごとに N 回問い合わせるのを避け,IN 句1本にまとめる)。
// targets が空なら空マップを返す。見つからない target はマップに現れない。
