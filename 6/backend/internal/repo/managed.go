...
type ManagedTable struct {
	...
	TableName      string
	DisplayName    string
	// Slug: URL(/tables/{slug})で使う管理名。全テーブル横断で一意(CI)。
	Slug        string
	Description string
...
const managedCols = `
	...
	m.display_name, m.slug, COALESCE(m.description, N''), m.pk_columns,
	m.readonly_columns, m.hidden_columns, m.fixed_columns, m.sort_order,
	m.enabled, m.created_by, m.created_at`
...
func scanManaged(row interface{ Scan(...any) error }) (*ManagedTable, error) {
	...
	err := row.Scan(&m.ID, &connID, &m.ConnectionName, &m.SchemaName, &m.TableName,
		&m.DisplayName, &m.Slug, &m.Description, &m.PKColumns,
...
func (r *Repo) CreateManagedTable(ctx context.Context, m ManagedTable) (*ManagedTable, error) {
	...
	err := r.queryRow(ctx, `
		INSERT INTO dbo.ftool_app_managed_tables
			(connection_id, schema_name, table_name, display_name, slug, description, pk_columns,
			 readonly_columns, hidden_columns, fixed_columns, sort_order, created_by, updated_by)
		OUTPUT INSERTED.id
		VALUES (@p1, @p2, @p3, @p4, @p5, @p6, @p7, @p8, @p9, @p10, @p11, @p12, @p12)`,
		connArg, m.SchemaName, m.TableName, m.DisplayName, m.Slug, nullIfEmpty(m.Description),
		m.PKColumns, m.ReadonlyColumns, m.HiddenColumns, fixed, m.SortOrder, m.CreatedBy).Scan(&id)
	if err != nil {
		if isUniqueViolation(err) {
			return nil, managedConflictErr(err, m)
		}
...
// managedConflictErr は一意制約違反を「どちらの制約か」で表示用に分類する
// (slug の一意インデックス名を含むかで判定)。
func managedConflictErr(err error, m ManagedTable) error {
	if strings.Contains(err.Error(), "ux_ftool_app_managed_tables_slug") {
		return fmt.Errorf("%w: 管理名 %s は既に使用されています", ErrConflict, m.Slug)
	}
	return fmt.Errorf("%w: %s.%s は既に登録されています", ErrConflict, m.SchemaName, m.TableName)
}

func (r *Repo) UpdateManagedTable(ctx context.Context, id int, displayName, slug, description *string, sortOrder *int, enabled *bool, readonlyColumns, hiddenColumns, fixedColumns *string, updatedBy string) (*ManagedTable, error) {
	var b setBuilder
	if displayName != nil {
	...
	if err != nil {
		if isUniqueViolation(err) && slug != nil {
			return nil, fmt.Errorf("%w: 管理名 %s は既に使用されています", ErrConflict, *slug)
		}
