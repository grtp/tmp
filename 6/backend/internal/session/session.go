// Package session stores login sessions in Redis, keyed by an opaque
// random session ID carried in an httpOnly cookie.
//
// 設計:
...
//   - Redis 断は fail-closed: ログインは 503、既存リクエストは 401 相当。
package session
...
// ErrNotFound = セッションが存在しない/失効した(ユーザー操作で回復可能)。
// それ以外のエラーは Redis 障害(インフラ)として扱う。
...
// Create issues a new session and returns its opaque ID.
...
// Get returns the claims for sid and slides the TTL forward (GETEX).
...
// SIDFromRequest extracts the session ID cookie ("" if absent).
...
// Cookie returns the Set-Cookie for a fresh session.
// Max-Age は付けない(ブラウザ側はセッションcookie扱い)。寿命の正は Redis の TTL。
...
// ExpiredCookie overwrites the cookie on logout.
