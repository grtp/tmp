...
│       ├─ repo/         アプリDB(ftool_app_ テーブル)のリポジトリ
...
│               initdb/prod-app-schema.sql(本番DDL,ftool_app_* のみ)
...
- **backend**: レイヤードアーキテクチャ。層の実体はディレクトリではなく
  **import グラフ**(`httpapi → {repo, store, meta, conn, audit, authz,
  ...
  `applog` はどの層からも使う横断的関心事(ログ)で例外的に葉として置く
- **httpapi 内の約束**: ハンドラは「認可 → 入力検証 → 委譲 → 監査 → 応答」に
  徹する。定型は `respond.go` のヘルパーを通す(`badRequest` / `internalError` /
  `auditOK` / `auditNG`)。認証前(ログイン)は claims がまだ無いため
  `auditAsOK` / `auditAsNG` で実行者を明示する。ファイルはドメイン単位
  (`users.go` / `actions.go` / `history.go` / `usersettings.go` …)
...
# 2) バックエンド(:8080)。設定は deploy/.env.dev(本番の deploy/.env と同じ形)
...
local に admin×3(tables / settings / history)を付与して使うのが基本。
...
2b. **ボタンはグローバル `.tm-btn`**(`libs/ui/src/styles/buttons.css`。
   修飾: `.primary` / `.danger`)を使う(2026-08 決定: ホーム設定ビルダーの
   ボタンを正として統一。Material の mat-*-button はテキストボタンには
   使わない。`mat-icon-button`(アイコンのみの行内操作)は例外として可)。
2c. **ツールバー等の flex 行にボタンを追加する時は `flex-shrink: 0` を
   明示する**(直接の `<button>` 子要素は `.toolbar > button` 等で既に
   保護されていることが多いが,`<span>` 等でラップした途端そのセレクタの
   対象から外れ,デフォルトの `flex-shrink:1` に戻る)。押し潰されると
   中の文言が折り返してツールバー自体が高くなり,**その下の要素(テーブル
   行など)の座標が想定よりずれる**。見た目のズレは数px で気付きにくいが,
   ドラッグ選択のような座標依存の操作が数行分ずれて壊れる形で症状が出る
   (実例: data-table-page.css `.sort-anchor`)。原因調査で「直前の
   コミットに戻すと直るか」を早めに確認すると特定が早い(`git stash` で
   一時的に作業中の変更を退避し,ベースラインと比較する)。
...
7. **ftool_app_ テーブルの DDL は `deploy/initdb/01_init.sql`(開発/検証,CREATE
   DATABASE/SCHEMA・サンプルデータ込み)と `deploy/initdb/prod-app-schema.sql`
   (本番,ftool_app_* のみ・CREATE DATABASE/SCHEMA 無し)の**両方**に同じ内容を
   反映する。新規 CREATE と,既存DB向けの**冪等 ALTER 追補**
   (`IF COL_LENGTH(...) IS NULL` 等)を必ずセットで書く。テーブル構成の全体像
   (ER図・命名規約)は [docs/db-schema.md](./db-schema.md) 参照。
   - ftool_app_ テーブルのスキーマは `$(APP_SCHEMA)` 変数で書く
     (`dbo.ftool_app_x` と直書きしない)。01_init.sql は冒頭の
     `:setvar APP_SCHEMA dbo` で既定値を持つが,prod-app-schema.sql は
     `:setvar` を持たず `sqlcmd -v APP_SCHEMA=` の指定を必須にしている
     (本番での dbo 決め打ち誤実行を防ぐため)
...
8. **アプリDBへの SQL は `dbo.ftool_app_xxx` と書き,必ず appdb.Q を通す**
   (repo/conn は Repo の `query/queryRow/exec` ヘルパ経由,tx 内は
   `appdb.Q(...)` を明示)。実行時に `dbo.` が `APP_DB_SCHEMA` の設定スキーマへ
   書き換えられる。アプリDBの SQL に ftool_app_ 接頭辞以外の意味で `dbo.` を
...
1. 変更箇所の特定
   - 見た目・DOM: `frontend/libs/ui/src/lib/<機能>/<component>/*.html|css|ts`
     (`<機能>` は `shared`/`login`/`tables`/`settings`/`history`。
...
| 画面 | libs/ui | コンテナ |
|---|---|---|
| ホーム | home-page / home-widget(ウィジェットレンダラ) | features/home(home-config.ts = JSON パース+権限フィルタ) |
| ホーム設定ビルダー | home-builder(パレット/キャンバス/インスペクタ) | features/settings/settings-home-container |
| テーブル管理(グリッド) | data-table-page, row-edit-dialog, csv-export/merge-dialog | features/tables |
| テーブル選択 | table-select-page | features/tables/table-select-container |
| 操作履歴 | history-page | features/history |
| 設定 | settings-page, managed-table-dialog, connection-dialog | features/settings |
...
2. `app.routes.ts` にルートを追加。**必ず `authGuard` + 必要なら
   ...
   入れてしまう)。
3. 画面へ入る導線を追加:
   - サイドバー: `shell-container.ts` の `menuItems`(権限チェック付き)
   - ホームの機能一覧: 機能として出すなら §4.5 の手順
...
3. backend 実装:
   - データアクセス: `internal/repo/`(ftool_app_ テーブル)または `internal/store/`
     (動的テーブル)
...
### 4.4 DB スキーマ(ftool_app_ テーブル)の変更

1. `deploy/initdb/01_init.sql` **と** `deploy/initdb/prod-app-schema.sql` の
   両方を変更(ftool_app_ テーブル部分は内容を同一に保つ)。テーブル構成の
   全体像は [docs/db-schema.md](./db-schema.md) 参照:
...
4. FK 設計の注意(SQL Server):
   - **複数カスケードパス**があると FK 作成自体が失敗する
     (同じテーブルへ2経路の CASCADE が届くケース)。その場合は NO ACTION にする
   - UNIQUE 制約を変える場合は旧制約の DROP を追補に含める
5. 監査履歴テーブル(`ftool_app_operation_history`)は**後方互換のまま**にする
   (既存行の書き換え・削除はしない)。

### 4.5 新しい組込機能(アクション)の追加

「機能」= ftool_app_actions の行 + ルート + 権限。UI からは追加・削除できない
(意図的。機能マスタの全行が常に組込)。

1. `01_init.sql` の ftool_app_actions MERGE シードに行を追加
   (`code`, 表示名, Material Symbols アイコン名, sort_order)。
...
3. frontend:
   ...
   - ホームの機能一覧(暫定実装)には権限があれば自動で出る
     (`features/home/home-container.ts`)
4. 権限付与は設定>ユーザー権限で(admin/maintainer/user の3段)。

### 4.6 接続まわりの改修

#### a. 業務DBの接続追加・変更(コード変更不要)
運用作業。設定 > テーブル管理設定 > 接続情報管理 で行う
...
変わると権限・個人設定の紐付けが切れる(移行時は
ftool_app_users.object_guid の突合・更新計画を立てること)。
...
2. 履歴 CSV の人間可読要約が必要なら
   ...
   (履歴画面のフィルタはチップフィルタで,機能は文字列部分一致のため
   コード追加は不要)
...
置く(例: `features/tables/csv-import.ts`)。
純粋かどうかとどこに属すかは別の軸。
...
注意:
- E2E は途中で管理テーブル・接続の作成/削除を行う。
  手動検証と並走させない(データが競合して誤爆する)
- シードデータに依存するチェックがある。シード元は2系統:
  - `01_init.sql` が自動登録: products / customers
  ...
  - `seizo-std` には kind='action'(tables)の項目が必須: テンプレート選択中は
...
- **`make dev_seed` は登録順が固定**(bigdata→wide→codes→audit_demo)。
  ...
  `/tables/6` と一致する。順序を変えたり間に別テーブルを挟むと
...
2. DDL 変更がある場合: `prod-app-schema.sql` の**追補ブロックだけ**を本番
   SQL Server へ適用(冪等なので再実行可。事前に ftool_app_ テーブルをバックアップ)
3. ビルドと反映:
   ```bash
...
- [ ] DDL は CREATE 本体+冪等追補の両方を書き,開発DBに適用した
      (`01_init.sql` / `prod-app-schema.sql` の ftool_app_ 部分を同一に保った)
