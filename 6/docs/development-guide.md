...
本書は F-tool(コード名 `f-tool`。アプリDBの既定データベース名 `tablemaint` は
互換のため未変更)へ機能追加・変更を行う開発者向けの手順書。
...
3. backend 実装:
   ...
   - `go build ./... && go test ./...`
4. frontend 実装:
   - 型: `apps/f-tool/src/app/core/models.ts`(OpenAPI を写す)
...
(`internal/httpapi/tables.go` の `connSchemaLimit`。登録候補 API/作成 API
...
fe-serve(:4200)が起動済み、かつ `make dev-seed` で demoDB 接続 +
...
- シードデータに依存するチェックがある。シード元は2系統:
  ...
  - `make dev-seed` が demoDB 接続経由で登録: bigdata / wide / codes / audit_demo
...
- **`make dev-seed` は登録順が固定**(bigdata→wide→codes→audit_demo)。
