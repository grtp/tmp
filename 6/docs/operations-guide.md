...
| `AUTO_GRANT_TABLES_VIEW` | true | 初回ログインで tables:user を自動付与 |
...
6. 管理者となるユーザーで一度ログイン(ftool_app_users に自動登録される)
   ...
   -- 対象ユーザーへ tables / settings(/history) の admin を MERGE で付与
   ```
...
| 業務テーブルのスキーマ | SELECT / INSERT / UPDATE / DELETE | テーブル管理の対象。**構造は変更させない**(DDL は与えない) |
...
- `AUTO_GRANT_TABLES_VIEW=true` なら閲覧権限(tables:user)が自動付与。
- 権限変更: 設定 > ユーザー権限(ユーザー×機能のマトリクス。
  admin > maintainer > user,空欄 = 権限なし)。
  - tables: user=閲覧,maintainer+=行の編集/CSV取込
...
### 4.4 ホーム画面の構成と機能の有効/無効

**ホーム画面の構成**は 設定 > ホーム設定(ビルダー)で編集する:

- 保存すると**全ユーザーのホーム画面に即時公開**される(確認あり)
- **既定に戻す**で設定を削除し,組込の既定表示(権限のある機能カード一覧)へ
- **JSON パネル**で構成の書き出し/貼り付け適用ができる。検証環境で作った
  構成を本番へ持ち運ぶ場合は,検証側で JSON をコピーし本番側で適用→保存する
- ウィジェットの「表示条件(権限)」を設定すると,その機能の権限が無い
  ユーザーには表示されない

機能の有効/無効は 設定 > 機能設定 で行う。
(旧「ホーム設定」= ダッシュボードテンプレート配布は S48(2026-08-04)で
廃止し,このビルダーに置き換えた。)
...
ディレクトリ移行で GUID が変わる場合,権限・個人設定
(`ftool_app_user_auth` / `ftool_app_user_settings`)の紐付けが切れる。移行時は
`ftool_app_users.object_guid` の新旧対応表を作り UPDATE する計画を立てること
(未対応のまま移行するとユーザーは「初回ログイン扱い」になり閲覧権限のみに戻る)。
