// Sprint 4 E2E: Sprint 3 全項目のリグレッション +
// CSV 出力(3スコープ/Excel互換BOM) / CSV 取込(精査/マージ画面/赤・オレンジ/
// 排除ボタン/適応) / (*)行 + まとめて保存 / ドラッグ範囲選択
// 前提: backend(run-dev-ldap 相当) + fe-serve + Edge headless(9223) 起動済み。
...
// S15: 削除/非表示(×)は画面編集モード中のみ表示されるため、カードの
...
async function exitDashboardEditMode(page) {
  await clickByText(page, '.edit-actions button', 'キャンセル');
  await sleep(300);
}
...
try {
  ...
  await page.goto(BASE + '/login', { waitUntil: 'networkidle2', timeout: 30000 });
  await sleep(400);
  check('login has lang pulldown', (await page.$('select')) !== null);
  ...
  // 表示と実言語の一致: forge.lang=en でリロードしてもプルダウンが English を指す
  // (旧実装は [value] バインディングの競合で「日本語」表示のまま英語UIになるバグ)
  await page.evaluate(() => localStorage.setItem('forge.lang', 'en'));
  ...
  check('lang pulldown matches actual language after reload', langState.selectValue === 'en' && langState.bodyEn);
  await page.evaluate(() => localStorage.setItem('forge.lang', 'ja'));
  await page.reload({ waitUntil: 'networkidle2' });
  await sleep(500);
  ...
  // 1) tanaka(admin) ログイン → サイドバー構成と FAB
  await page.type('input[type="text"]', 'tanaka');
  await page.type('input[type="password"]', 'Passw0rd!');
  await clickByText(page, 'button', 'ログイン');
  await page.waitForFunction(() => location.pathname === '/dashboard', { timeout: 10000 });
  await sleep(600);

  // 状態を既定にリセット(前回実行や手動検証の残骸を排除)
  await page.evaluate(async () => {
    await fetch('/api/v1/me/dashboard', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ templateId: null, cardOrder: null }),
    });
    // 個人カード(リンク/テーブル)も全削除して開始する
    const r = await fetch('/api/v1/me/links').then((x) => x.json());
    for (const l of r.links) {
      await fetch('/api/v1/me/links/' + l.id, { method: 'DELETE' });
    }
  });
  await page.reload({ waitUntil: 'networkidle2' });
  await sleep(600);
  ...
  check('no settings/history cards', !cardNames.some((n) => n.includes('設定') || n.includes('操作履歴')));
  check('FAB exists', (await page.$('button.fab')) !== null);
  ...
  // 4) 画面編集(旧:並び替え): FAB→[画面編集]でモード開始 → D&D → 決定で保存 / キャンセルで破棄
  // S15: 並び替えと同時に削除もできるようにした(旧仕様は編集モード中は削除ボタン非表示)
  const orderBefore = await page.evaluate(() =>
    [...document.querySelectorAll('.card')].map((c) => c.getAttribute('data-key')),
  );
  ...
  const cardKeys = () =>
    page.evaluate(() => [...document.querySelectorAll('.card')].map((c) => c.getAttribute('data-key')));

  // 4a) キャンセル: 並びを変えてもキャンセルで元に戻り、保存されない
  await page.click('button.fab');
  await sleep(300);
  await clickByText(page, 'tm-add-feature-dialog button.entry', 'テンプレート');
  await sleep(300);
  await clickByText(page, 'tm-add-feature-dialog button.entry', '画面編集');
  await sleep(300);
  check('edit mode shows cancel/confirm', (await page.$('.edit-actions')) !== null);
  // S15: 画面編集モード中も削除ボタン(link-ops)が描画され、ドラッグに巻き込まれないよう
  ...
  check(
    'edit mode still shows delete/hide button on cards',
  ...
  await clickByText(page, '.edit-actions button', 'キャンセル');
  await sleep(300);
  check('cancel restores original order', JSON.stringify(await cardKeys()) === JSON.stringify(orderBefore));
  check('cancel leaves edit mode (FAB back)', (await page.$('button.fab')) !== null);
  ...
  await dragLastToFirst();
  await sleep(300);
  const orderPreview = await cardKeys();
  check(
    'drag preview moves last card to front',
    orderPreview[0] === orderBefore[orderBefore.length - 1] && orderPreview.length === orderBefore.length,
  );
  await clickByText(page, '.edit-actions button', '決定');
  await sleep(800); // PUT /me/dashboard
  check('confirm leaves edit mode (FAB back)', (await page.$('button.fab')) !== null);
  await page.reload({ waitUntil: 'networkidle2' });
  await sleep(800);
  check('card order persists after reload', JSON.stringify(await cardKeys()) === JSON.stringify(orderPreview));
  ...
  await page.waitForFunction(() => document.body.innerText.includes('10,000+'), { timeout: 15000 });
  check('capped count shows 10,000+', true);
  check('keyword search box removed', (await page.$('input[type="search"]:not(.filter-input)')) === null);

  // 列幅(S6): 全実列は内容幅で固定、末尾の疑似カラム(フィラー)が余白を
  ...
  // 単独リサイズ(S6): ドラッグした列だけが変わり、他列とテーブル全幅は不変
  // (伸縮はフィラーが引き受ける)
  const bigdataCols = () =>
    page.evaluate(() => {
      const ths = [...document.querySelectorAll('thead tr:first-child th:not([data-tm-filler])')];
  ...
  // ウィンドウ縮小(S6 の不具合報告シナリオ): フィラーが縮むだけで
  ...
  await page.evaluate(() => localStorage.removeItem('forge.colw:1:dbo.bigdata'));
  await page.reload({ waitUntil: 'networkidle2' });
  await page.waitForFunction(() => document.body.innerText.includes('10,000+'), { timeout: 15000 });
  ...
  await page.waitForFunction(() => document.body.innerText.includes('10,000+'), { timeout: 10000 });
  check('chip remove restores capped count', (await page.$('.chip')) === null);
  ...
  check('drag does not open edit dialog', (await page.$('tm-row-edit-dialog .dialog')) === null);
  ...
  check(
    ...
    (await page.$$('.check-td input:checked')).length === 1 &&
      (await page.$('tm-row-edit-dialog .dialog')) === null,
  );
  await page.mouse.click(p3.x, p3.y);
  await sleep(200);
  ...
  check(
    ...
    (await page.evaluate(() => document.querySelector('p.error')?.textContent ?? '')).includes('nope') &&
      (await page.$('tm-csv-merge-dialog .dialog')) === null,
  );
  ...
  await sleep(500);
  check('merge dialog opens', (await page.$('tm-csv-merge-dialog .dialog')) !== null);
  ...
  await page.evaluate(() => localStorage.removeItem('forge.colw:history'));
  ...
  await page.mouse.up();
  await sleep(300);
  const stored = await page.evaluate(() => localStorage.getItem('forge.colw:history'));
  ...
  // 履歴の CSV 出力(サーバー生成。BOM なし・JST ヘッダー・detail 列)
  await page.evaluate(() => {
    window.__csvTexts = [];
    const orig = URL.createObjectURL.bind(URL);
    URL.createObjectURL = (b) => {
  ...
  check(
    'history CSV export: JST header + summary/detail columns, no BOM',
    !histCsv.startsWith('﻿') &&
  ...
  const settingsRow = await page.evaluate(() => {
    ...
    return {
      ...
      hasLock: row?.querySelector('.ti-lock') !== null,
    };
  });
  ...
  await clickByText(page, 'button', 'テンプレート');
  await sleep(400);
  ...
  // 8.6) S5: サイドバー「テーブルメンテ」/ テーブルカード / 列モード / 前後値
  // 列モードの検証は demo.dbo.audit_demo(id=6, phone=除外, updated_at=編集不可)
  // を登録済みであることに依存する。
  await page.goto(BASE + '/dashboard', { waitUntil: 'networkidle2' });
  await sleep(600);
  const s5menu = await page.evaluate(() =>
    [...document.querySelectorAll('.menu-item')].map((b) => b.textContent?.trim() ?? ''),
  );
  check('sidebar has table-maint item', s5menu.some((m) => m.includes('テーブルメンテ')));
  ...
  // テーブルカード(S6 階層): (+) → 機能へのショートカット追加 → テーブルメンテ →
  ...
  const findTableCard = () =>
    page.evaluate(
      () =>
        [...document.querySelectorAll('.card.personal')].some((x) =>
          x.textContent?.includes('商品マスタ'),
        ),
    );
  check('personal table card appears', await findTableCard());
  // S6-6: カード粒度統一 — 1行目=機能名 / 2行目(詳細)=テーブル名
  const tableCardShape = await page.evaluate(() => {
    const c = [...document.querySelectorAll('.card.personal')].find((x) =>
      x.textContent?.includes('商品マスタ'),
    );
    return c
      ? {
  ...
  check('rows.batch audit records before/after', s5HasBefore);

  // 8.7) S6: 機能ショートカットカード / 非個人カードの非表示と復元 / 新規グレーアウト
  await page.goto(BASE + '/dashboard', { waitUntil: 'networkidle2' });
  await sleep(600);
  await page.click('button.fab');
  await sleep(300);
  ...
  check('personal function shortcut card appears', await hasShortcutCard());
  await page.evaluate(() => {
    const c = [...document.querySelectorAll('.card.personal')].find((x) =>
      x.textContent?.includes('テーブルメンテナンス'),
    );
  ...
  await sleep(800);
  check('shortcut card removed', !(await hasShortcutCard()));

  // 非個人カード(既定の機能カード)の×で非表示 → hiddenKeys 保存 → 既定を再選択で復元
  // (画面編集モードは継続中。× は edit モード中のみ表示される)
  const hideTarget = await page.evaluate(() => {
    const c = [...document.querySelectorAll('.card')].find(
      (x) => !x.classList.contains('personal') && x.getAttribute('data-key')?.startsWith('fn:'),
    );
    if (!c) return null;
    // 非個人(テンプレ由来/既定)カードの非表示ボタンは警告色(.warn)、
    // 個人カードの実削除ボタンは危険色(.danger)
    c.querySelector('.link-ops .warn')?.dispatchEvent(new MouseEvent('click', { bubbles: true }));
    return c.getAttribute('data-key');
  });
  await sleep(800);
  const hiddenState = await page.evaluate(async () => {
    const dash = await fetch('/api/v1/me/dashboard').then((x) => x.json());
    return dash.hiddenKeys ?? [];
  });
  check('hide card persists hiddenKeys', hideTarget !== null && hiddenState.includes(hideTarget));
  check(
    'hidden card disappears',
    await page.evaluate(
      (key) => ![...document.querySelectorAll('.card')].some((c) => c.getAttribute('data-key') === key),
      hideTarget,
    ),
  );
  // 既定(現在の選択)を選び直す = 復元(通常の FAB に戻すため画面編集モードを抜ける)
  ...
  await sleep(900);
  check(
    'template re-select restores hidden card',
    await page.evaluate(
      (key) => [...document.querySelectorAll('.card')].some((c) => c.getAttribute('data-key') === key),
      hideTarget,
    ),
  ...
  // 8.8) S6-7: 管理テーブルの行クリック → 編集ダイアログ(表示名/説明/列モード)
  await page.goto(BASE + '/settings/table-maint', { waitUntil: 'networkidle2' });
  await sleep(600);
  await clickByText(page, 'button', '管理テーブル');
  await sleep(500);
  await page.evaluate(() => {
  ...
  // 8.9) S6-8: リンク追加は「機能リンク」配下のフォーム(← で戻れる) +
  ...
  await page.evaluate(() => {
    ...
    const setter = Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value').set;
    setter.call(inputs[0], 'e2e-s6-link');
    inputs[0].dispatchEvent(new Event('input', { bubbles: true }));
    setter.call(inputs[1], 'https://example.com/s6');
    inputs[1].dispatchEvent(new Event('input', { bubbles: true }));
  });
  await clickByText(page, 'tm-add-feature-dialog .form-foot button', '追加');
  await sleep(900);
  const s6Link = await page.evaluate(() => {
    const c = [...document.querySelectorAll('.card')].find((x) =>
      x.textContent?.includes('e2e-s6-link'),
    );
    return c
      ? { detail: c.querySelector('.card-detail')?.textContent?.trim() }
      : null;
  ...
  check(
    ...
    !!s6Link && s6Link.detail === 'https://example.com/s6',
  );
  await page.evaluate(async () => {
    const r = await fetch('/api/v1/me/links').then((x) => x.json());
    const mine = r.links.find((l) => l.name === 'e2e-s6-link');
    if (mine) await fetch('/api/v1/me/links/' + mine.id, { method: 'DELETE' });
  });

  // 言語切替: 組込機能のカード名が英語に追従し、個人リンク名は翻訳されない
  await page.reload({ waitUntil: 'networkidle2' });
  await sleep(700);
  await page.select('tm-lang-select select', 'en');
  await sleep(600);
  ...
  // S15: 設定>ユーザー権限/機能タブでも組込機能名が言語切替に追従すること
  ...
  await page.goto(BASE + '/settings/dashboard', { waitUntil: 'networkidle2' });
  await sleep(500);
  await clickByText(page, '.tab', 'Functions');
  await sleep(300);
  ...
  await page.select('tm-lang-select select', 'ja');
  await sleep(600);

  // 8.10) S7: ヘッダーロゴ / 個人テンプレート(保存→バッジ→適用→削除)
  await page.goto(BASE + '/dashboard', { waitUntil: 'networkidle2' });
  await sleep(600);

  // S14: サイドバー開閉に連動してヘッダーは テキストのみ/アイコンのみ を切り替える
  ...
  const menuLabelsHidden = await page.evaluate(() =>
    [...document.querySelectorAll('.menu-item')].every((el) => el.textContent?.trim() === ''),
  );
  check('sidebar collapsed: menu labels hidden', menuLabelsHidden);
  check(
    'sidebar collapse persisted to localStorage',
    (await page.evaluate(() => localStorage.getItem('forge.sidebarCollapsed'))) === '1',
  );
  await page.click('.sidebar-toggle');
  await sleep(300);
  check(
  ...
  const savedTpl = await page.evaluate(async () => {
    ...
    return { applied: !!my && dash.templateId === my.id, personal: my?.personal === true };
  });
  check('save current layout creates + applies personal template', savedTpl.applied && savedTpl.personal);
  ...
  check(
    ...
    !!myEntry && myEntry.text.includes('個人') && myEntry.del && myEntry.selected &&
      !!distEntry && distEntry.text.includes('配布') && !distEntry.del,
  );
  // 個人テンプレートの削除 → 一覧から消え、選択中だったので既定へ戻る
  await page.evaluate(() => {
  ...
  const afterDel = await page.evaluate(async () => {
    ...
    return {
      ...
      fallback: dash.templateId === null,
    };
  });
  check('personal template delete falls back to default', afterDel.gone && afterDel.fallback);
  await page.keyboard.press('Escape');
  await sleep(200);
  await page.keyboard.press('Escape');
  await sleep(200);
  ...
  // 8.11) S9: 接続のスキーマ制限(app_connections.schema_name)。
  ...
  const restrictedConn = await page.evaluate(async () => {
    const res = await fetch('/api/v1/connections', {
      ...
      body: JSON.stringify({
        ...
        username: 'sa', password: 'DevPassw0rd!', schemaName: 'e2e_schema',
      }),
    });
  ...
  await sleep(600);
  await page.evaluate(() => {
    [...document.querySelectorAll('button.tab')].find((b) => b.textContent?.includes('接続'))?.click();
  });
  await sleep(500);
  ...
  await page.evaluate(() => {
    [...document.querySelectorAll('button.tab')].find((b) => b.textContent?.includes('管理テーブル'))?.click();
  });
  await sleep(500);
  ...
  await logout(page);
  await login(page, 'sato', 'Passw0rd!');
  ...
  try {
    await page.screenshot({ path: 'e2e-s3-failure.png' });
    console.log('screenshot saved: e2e-s3-failure.png');
  } catch { /* ignore */ }
} finally {
