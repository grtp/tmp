// features/settings — 設定>テーブル管理(/settings/tables)。
...
export class SettingsTablesContainer {
  ...
  private readonly run = createReloadRunner(
    this.transloco,
    this.saving,
    this.errorMessage,
    (silent) => this.reload(silent),
  );
  ...
  /** 「既に使用されています」即時表示用の使用済み管理名(小文字化。編集中の自分は除く)。 */
  ...
  private openManagedTableDialog(
    init: { mode: 'create' | 'edit'; editValue: ManagedTableEditValue | null },
    ...
    // edit モードは実テーブルの接続が確定しているので候補は取得不要,
    // プレビューだけ取りに行く(create モードは candidateSelected 経由)。
    const editConnId =
      editingTableId !== null
    ...
    const loadPreview = async (
      ...
      try {
        const p = await this.admin.previewSchemaTable(
          schemaName,
          tableName,
          currentConnId,
        );
    ...
    ref.componentInstance.candidateSelected.subscribe(
      (e: { schemaName: string; tableName: string }) => {
    ...
    ref.componentInstance.confirmed.subscribe((e: ManagedTableRegistration) => {
      const fallbackKey =
        editingTableId !== null ? 'errors.saveFailed' : 'errors.registerFailed';
      void runDialogAction(this.transloco, ref, fallbackKey, async () => {
        if (editingTableId !== null) {
          // 編集: 表示名/管理名/説明/列モードのみ更新(接続・実テーブルは変更不可)。
    ...
    ref.componentInstance.saved.subscribe((e: ConnectionSubmit) => {
      void runDialogAction(this.transloco, ref, 'errors.saveFailed', async () => {
        if (data.mode === 'create') {
          ...
          // mode: 'edit' は openConnectionEdit が editingConnId を設定してから
          // 開くため常に非null(create/edit は同じ subscribe を共有するための型)。
    ...
    ref.componentInstance.testClicked.subscribe((e: ConnectionSubmit) => {
      void (async () => {
        ...
        try {
          ...
          ref.componentRef?.setInput(
            'testResult',
            formatTestResult(this.transloco, result),
          );
        } catch (err) {
          ref.componentRef?.setInput(
            'testResult',
            apiErrorText(this.transloco, err, 'errors.testFailed'),
          );
        } finally {
  ...
  protected async onConnectionTest(id: number): Promise<void> {
    ...
    try {
      ...
      this.snackBar.open(
        `${name}: ${formatTestResult(this.transloco, result)}`,
        undefined,
        {
      ...
      this.snackBar.open(
        `${name}: ${apiErrorText(this.transloco, err, 'errors.testFailed')}`,
        undefined,
        { duration: 6000, panelClass: 'toast-ng' },
      );
    }
  }
