// features/tables — メタデータ駆動のテーブル管理画面(1テーブル分)。
//
// ルート /tables/:id で表示する。テーブルの選択はカード画面
...
export class TablesContainer implements ConfirmsLeave {
  ...
  protected readonly writable = computed(
    () =>
      this.auth.allows('tables', 'maintainer') &&
  ...
  /** 並べ替え状態([ソート]タグ経由)。テーブル切替時に core reset される。 */
  protected readonly sortKey = signal<string | null>(null);
  protected readonly sortDir = signal<'asc' | 'desc'>('asc');

  protected async onSortChanged(e: { key: string; dir: 'asc' | 'desc' }): Promise<void> {
  ...
  /**
   * ルートの :id(管理名 slug)を内部 id へ解決する(大文字小文字は
   * 区別しない)。URL は slug のみ有効(数値 id は slug が英字始まり必須の
   * ため何にも一致せず,一覧へ戻る)。API は数値 id のみを受けるため,
   * URL 層だけの解決で完結する。
   */
  private async resolveTableId(): Promise<number> {
    ...
    if (!hit) throw new Error(`unknown table slug: ${this.idParam}`);
    return hit.id;
  }
