...
export interface ManagedTableDialogData {
  ...
  /**
   * 使用済みの管理名(小文字化済み。edit では自分自身を除く)。
   * 入力中の「既に使用されています」即時表示に使う。
   */
  usedSlugs: string[];
}
...
/** 管理名の形式(バックエンドと同一)。英字始まり = 数値IDのURL空間と衝突しない。 */
...
export class ManagedTableDialog {
  ...
  /**
   * 管理名の入力エラー(即時表示)。invalid = 形式違反,taken = 使用済み。
   * 空はエラー表示しない(canConfirm で保存だけ止める)。
   */
  protected readonly slugError = computed<'invalid' | 'taken' | null>(() => {
    const s = this.slug().trim();
    if (s === '') return null;
    if (!SLUG_RE.test(s)) return 'invalid';
    if (this.data.usedSlugs.includes(s.toLowerCase())) return 'taken';
    return null;
  });
  ...
  protected readonly canConfirm = computed(() => {
    const named =
      ...
      this.slugError() === null;
    const base =
      this.mode === 'edit' ? named : this.selected() !== null && named;
  ...
  protected select(t: CandidateTable): void {
    ...
    // 管理名は実テーブル名を初期提案する(形式外の名前なら手で直してもらう)。
    if (this.slug().trim() === '') {
  ...
  protected confirm(): void {
    const v = this.mode === 'edit' ? this.editValue : null;
    const s = this.selected();
    // 編集モードは固定対象(v)を優先し,新規登録では選択中の候補(s)を使う。
    const target = v ?? s;
    if (!target) return;
    const modes = this.colModes();
