...
export interface ColumnDef {
  ...
  mono?: boolean;
  /** 'right' で見出し・値とも右寄せ(数値列向け。桁の比較がしやすくなる) */
  align?: 'left' | 'right';
  /** true でヘッダークリックによる並べ替えを受け付ける(状態はコンテナが持つ) */
  sortable?: boolean;
  /**
 ...
 * テーブル管理だけでなく操作履歴・ユーザー一覧も同じ部品を使う前提で,
...
export class DataTablePage {
  ...
  /** 並べ替え中の列キー(null = 未適用)。sortable な列にだけ意味を持つ */
  ...
  /** 並べ替え変更([ソート]ボタンの一覧から列を選ぶと発火) */
  readonly sortChanged = output<{ key: string; dir: 'asc' | 'desc' }>();
  /** 並べ替えタグの × で解除(未ソートに戻す) */
  ...
  /** フィルタのポップオーバーが開いている間 true([フィルタ]の active 表示用) */
  protected readonly filterPopOpen = signal(false);

  /**
   * ツールバーの[フィルタ]: 押したボタンをアンカーに列選択ポップオーバーを
   * 直接開く。チップ行は条件が確定するまで視覚的に出さない(bare)ため,
   * ここでは開くだけでよい(tm-filter-bar は常時マウント)。
   */
  protected onFilterButton(e: Event): void {
    // このクリックが document へ伝播すると filter-bar の「外側クリックで
    // 閉じる」処理が開いた直後のポップオーバーを閉じてしまうため止める
    e.stopPropagation();
  ...
  protected onFilterOpenChanged(open: boolean): void {
    ...
    // フィルタとソートのポップオーバーが同時に開いて重なるのを防ぐ
    if (open) this.sortPop.set(null);
  }

  protected readonly sortableColumns = computed(() => this.columns().filter((c) => c.sortable));

  /**
   * 並べ替えのポップオーバー(フィルタの列選択ポップオーバーと同じ見た目:
   * 検索欄+列一覧。位置はビューポート座標の fixed。トリガー(ツールバーの
   * [ソート]ボタン,またはタグ本体)の真下中央に,パネル内へクランプして出す)。
   */
  protected readonly sortPop = signal<{ left: number; top: number } | null>(null);
  ...
  private sortAnchorPos(el: HTMLElement): { left: number; top: number } {
  ...
  /**
   * 並べ替えポップオーバーを開く(ツールバーの[ソート]ボタン,タグ本体の
   * どちらからでも同じ処理)。document クリックへ伝播すると開いた直後の
   * ポップオーバーが自分自身の外側クリック判定で閉じてしまうため止める。
   */
  ...
  /**
   * 列を選んで並べ替えを適用。別列を選べば昇順から,同じ列を選び直すたびに
   * 昇順/降順をトグルする。
   */
  ...
  protected onEscapeForSort(): void {
    if (this.sortPop()) this.sortPop.set(null);
  }
  ...
  /** ツールバーの[全選択]: 全選択中なら解除,そうでなければ全選択(旧ヘッダー
   * チェックボックスと同じ二値挙動)。 */
  ...
  /** ドラッグ開始時点の選択(このドラッグの範囲外は常にこのまま保つ)。 */
  private dragBase: ReadonlySet<number> = new Set();
  /** 起点行が開始時に選択済みなら解除方向, 未選択なら追加方向。 */
  ...
  protected onRowPointerDown(index: number, e: PointerEvent): void {
    ...
    // 起点行の「開始時点の状態」で追加/解除を決める(前回までの選択は
    // ドラッグ範囲外である限りそのまま残す。既存選択済みの箇所を
    // ドラッグすると解除,未選択の箇所をドラッグすると追加になる)。
  ...
  protected onRowPointerEnter(index: number): void {
    ...
    for (let i = from; i <= to; i++) {
      if (this.dragMode === 'add') next.add(i);
      else next.delete(i);
    }
