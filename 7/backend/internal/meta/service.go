...
WHERE m.id = @p1`
	...
	def.Columns, def.InsertBlockedColumns, def.FixedColumns =
...
// 第3戻り値は正規化済みの固定値指定(実在し PK/hidden/自動readonly でない列のみ)。
func applyColumnModes(cols []Column, pk []string, roCSV, hdCSV string, fixed []FixedColumn) ([]Column, []string, []FixedColumn) {
	...
	for _, c := range cols {
		...
		if _, isPK := pkSet[key]; !isPK {
			...
			if f, isFixed := fx[key]; isFixed && !c.Readonly {
				c.Readonly = true
				c.Required = false
				c.Fixed = true
