...
// 管理対象は app_managed_tables に登録された schema.table。
package meta
...
// (app_managed_tables.fixed_columns の1要素)。
...
// TableDef は実行時のテーブル定義: app_managed_tables の行 +
// 最新のカタログイントロスペクション。
type TableDef struct {
	...
	// FixedColumns: 固定値列の指定(実在し PK/hidden でないものだけに正規化済み)。
	// store が insert/update 時に値を自動セットする。
