// Package repo は本ツール自身の app_* テーブルの CRUD を担う:
...
// 機能(行)の追加 API は存在しない: コード実装 + シードとセットで行う。
// 任意 URL のリンクカードは個人機能(app_user_links)に統合された。
...
func (r *Repo) DeleteAction(ctx context.Context, id int) error {
	...
	if _, err := r.exec(ctx, `DELETE FROM dbo.app_actions WHERE id = @p1`, id); err != nil {
		return fmt.Errorf("repo: delete action: %w", err)
	}
	return nil
}
