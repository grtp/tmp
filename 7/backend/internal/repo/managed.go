	...
	err := r.queryRow(ctx, `
		INSERT INTO dbo.app_managed_tables
			(connection_id, schema_name, table_name, display_name, description, pk_columns,
			 readonly_columns, hidden_columns, fixed_columns, sort_order, created_by)
		OUTPUT INSERTED.id
...
func (r *Repo) UpdateManagedTable(ctx context.Context, id int, displayName, description *string, sortOrder *int, enabled *bool, readonlyColumns, hiddenColumns, fixedColumns *string) (*ManagedTable, error) {
	var b setBuilder
	if displayName != nil {
