...
func TestFixedValue(t *testing.T) {
	...
	t.Run("now for datetime returns UTC time", func(t *testing.T) {
		got, err := fixedValue(meta.FixedColumn{Kind: "now"}, col(meta.TypeDateTime))
		if err != nil {
			t.Fatalf("now: %v", err)
		}
		tm, ok := got.(time.Time)
		if !ok || tm.Location() != time.UTC {
			t.Errorf("now -> %v (%T), want UTC time.Time", got, got)
		}
	...
	t.Run("now for string returns RFC3339", func(t *testing.T) {
		got, err := fixedValue(meta.FixedColumn{Kind: "now"}, col(meta.TypeString))
		if err != nil {
			t.Fatalf("now/string: %v", err)
		}
		if _, perr := time.Parse(time.RFC3339, got.(string)); perr != nil {
			t.Errorf("now/string not RFC3339: %v", got)
		}
	})
