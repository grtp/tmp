...
import { arrangeCards } from './dash-cards';
...
export class DashboardContainer {
  ...
  protected readonly cards = computed<DashCard[]>(() => {
    ...
    // 非表示フィルタ(テンプレ再選択で復元)+ 保存済み並び順の寛容マージ。
    return arrangeCards(base, this.cardOrder(), this.hiddenKeys());
  });
  ...
  private async init(): Promise<void> {
    ...
    // spec 上は省略可(省略 = 未設定)なので null に正規化して保持する。
    const templateId = dash.templateId ?? null;
    this.cardOrder.set(dash.cardOrder ?? null);
    this.hiddenKeys.set(dash.hiddenKeys ?? []);
    this.templateId.set(templateId);
    if (templateId !== null) {
      try {
        this.template.set(await this.dashApi.getTemplate(templateId));
      } catch {
