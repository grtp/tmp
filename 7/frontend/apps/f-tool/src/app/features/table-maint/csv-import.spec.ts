...
import { CSV_IMPORT_MAX_ROWS, CsvImportRow, markConflicts, validateCsvRecords } from './csv-import';
...
function tableMeta(columns: ColumnMeta[], primaryKey: string[] = ['id']): TableMeta {
  return {
    id: 1,
    displayName: 't',
    primaryKey,
...
describe('validateCsvRecords', () => {
  it('ヘッダーのみ / 空入力は中止する', () => {
    expect(validateCsvRecords([], meta, labels)).toEqual({ ok: false, error: 'EMPTY' });
    expect(validateCsvRecords([['id', 'name']], meta, labels)).toEqual({ ok: false, error: 'EMPTY' });
  });

  it('上限行数ちょうどは許可し，超過は中止する', () => {
    const header = ['name'];
    const atCap = [header, ...Array.from({ length: CSV_IMPORT_MAX_ROWS }, () => ['x'])];
    expect(validateCsvRecords(atCap, meta, labels).ok).toBe(true);
    const overCap = [...atCap, ['y']];
    expect(validateCsvRecords(overCap, meta, labels)).toEqual({ ok: false, error: 'TOO_MANY' });
  });
  ...
  it('列数不一致は 1-origin + ヘッダー行込みの行番号で中止する', () => {
    ...
    expect(validateCsvRecords(records, meta, labels)).toEqual({ ok: false, error: 'COUNT:3' });
  });
  ...
  it('型ごとにセルを解釈する(int/decimal/bool/date/uuid/空=null)', () => {
    ...
    expect(r.rows[1].parsed).toEqual({ name: 'b', qty: null, price: null, active: false, due: null, guid: null });
  });
...
describe('markConflicts', () => {
  const writableMeta = tableMeta(
    [col({ name: 'code', required: true, nullable: false }), col({ name: 'name' })],
    ['code'],
  );

  it('フェッチ済み行と主キー一致で conflict を立てる(大小・空白は無視)', () => {
    const rows = [importRow({ code: ' ABC ' }), importRow({ code: 'zzz' })];
    const n = markConflicts(rows, writableMeta, [{ code: 'abc' }, { code: 'def' }]);
    expect(n).toBe(1);
