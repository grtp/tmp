...
export type ColumnMode = 'edit' | 'readonly' | 'hidden' | 'fixed';
  ...
  constructor() {
    ...
    effect(() => {
      ...
      if (p) {
        ...
        const fx = new Map(
          (v?.fixedColumns ?? []).map((f) => [f.name.toLowerCase(), f] as const),
        );
        for (const c of p.columns) {
          ...
          if (v) {
            ...
            if (f) {
              init[c.name] = 'fixed';
              drafts[c.name] = { kind: f.kind, value: f.value ?? '', applyOn: f.applyOn };
            } else {
  ...
  protected setMode(name: string, mode: string): void {
    ...
    if (mode === 'fixed') {
      this.fixedDrafts.update((d) =>
        d[name] ? d : { ...d, [name]: { kind: 'literal', value: '', applyOn: 'both' } },
      );
    }
  }

  protected fixedDraftOf(name: string): FixedDraft {
    return this.fixedDrafts()[name] ?? { kind: 'literal', value: '', applyOn: 'both' };
  }

  protected setFixedDraft(name: string, patch: Partial<FixedDraft>): void {
    this.fixedDrafts.update((d) => ({ ...d, [name]: { ...this.fixedDraftOf(name), ...patch } }));
  }
